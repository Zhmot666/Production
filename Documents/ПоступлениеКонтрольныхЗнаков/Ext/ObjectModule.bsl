...
Функция ПроверкаПриходаКЗ(Серия, НачальныйНомер, КоличествоКонтрольныхЗнаков)
	...
	//ТаблицаКЗ = Новый ТаблицаЗначений;
	//ТаблицаКЗ.Колонки.Добавить("СерияКЗ", Тип("Строка"));
	//ТаблицаКЗ.Колонки.Добавить("НомерКЗ", Тип("Число"));
	//
	//Для НомерКЗ = НачальныйНомер По НачальныйНомер + КоличествоКонтрольныхЗнаков - 1 Цикл
	//	НоваСТрокаТЗ = ТаблицаКЗ.Добавить();
	//	НоваСТрокаТЗ.СерияКЗ = Серия;
	//	НоваСТрокаТЗ.НомерКЗ = НомерКЗ;
	//КонецЦикла; 
	//
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	...
	//|			ТКЗ.СерияКЗ КАК СерияКЗ,
	//|			ТКЗ.НомерКЗ КАК НомерКЗ
	//|		ИЗ
	//|			ТКЗ КАК ТКЗ) КАК ВложенныйЗапрос
	//|		ПО ОстаткиКЗ.Номер = ВложенныйЗапрос.НомерКЗ
	//|			И ОстаткиКЗ.Серия = ВложенныйЗапрос.СерияКЗ
	//|ГДЕ
	//|	ОстаткиКЗ.Серия = &Серия
	//|	И ОстаткиКЗ.Номер = &Номер";
	//
