...
Процедура АктОтбораПроб1(ТабДок, Ссылка) Экспорт
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		...
		|	Списание.РасходНаИспытания КАК РасходНаИспытания,
		|	Списание.Потери КАК Потери,
		|	Списание.ВозвращеноВТЦ КАК ВозвращеноВТЦ
		|ИЗ
		...
		ОбластьДвижениеМасла = Макет.ПолучитьОбласть("Расход");
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока ВыборкаПробы.Следующий() Цикл
				...
				ПоследнийНомерСтроки = ВыборкаПробы.НомерСтроки;
			КонецЦикла;
			Если Выборка.РасходНаИспытания > 0 или Выборка.Потери > 0 или Выборка.ВозвращеноВТЦ > 0 Тогда
				ОбластьДвижениеМасла.Параметры.НомерСтроки = ПоследнийНомерСтроки + 1;
				ОбластьДвижениеМасла.Параметры.ВидДвиженияМасла = "Расход на испытания";
				ОбластьДвижениеМасла.Параметры.КоличествоДвижения = Выборка.РасходНаИспытания;
				ТабДок.Вывести(ОбластьДвижениеМасла);

				ОбластьДвижениеМасла.Параметры.НомерСтроки = ПоследнийНомерСтроки + 2;
				ОбластьДвижениеМасла.Параметры.ВидДвиженияМасла = "Потери";
				ОбластьДвижениеМасла.Параметры.КоличествоДвижения = Выборка.Потери;
				ТабДок.Вывести(ОбластьДвижениеМасла);
				
				ОбластьДвижениеМасла.Параметры.НомерСтроки = ПоследнийНомерСтроки + 3;
				ОбластьДвижениеМасла.Параметры.ВидДвиженияМасла = "Возвращение в технологический цикл";
				ОбластьДвижениеМасла.Параметры.КоличествоДвижения = Выборка.ВозвращеноВТЦ;
				ТабДок.Вывести(ОбластьДвижениеМасла);
