	...
	Процедура ОбработкаПроведения(Отказ, Режим)
		...
		Для Каждого ТекСтрокаПробы Из Пробы Цикл
			...
			Если ТекСтрокаПробы.ЦельИсследования = Справочники.ЦелиИсследований.Арбитраж Тогда
				...
				Движение.Период = Дата;
				Движение.Номенклатура = ТекСтрокаПробы.НаименованиеПроб;
				ПоследняяНоменклатура = ТекСтрокаПробы.НаименованиеПроб;
		...
		//Движение остатков масла
		Движение = Движения.Остатки.Добавить();
		Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
		Движение.Период = Дата;
		Движение.Номенклатура = ВидМасла;
		Движение.Склад = СкладВозвратаМасла;
		Движение.Количество = Окр(ВозвращеноВТЦ/100*ОбщегоНазначенияСервер.ОпределитьПроцентВозврата(ПоследняяНоменклатура), 2);
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.Производство") Тогда
			...
			Если КоличествоЗначащихСтрок > 0 Тогда
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Если Строка(ТипЗнч(ОбщегоНазначенияСервер.ЕстьВозратМасла(ВыборкаДетальныеЗаписи.Продукция))) = "Число" Тогда
						СкладВозвратаМасла = ДанныеЗаполнения.СкладСписанияСырья;
					КонецЕсли;
					ВозвратноеМасло = ОбщегоНазначенияСервер.УзнатьПолуфабрикат(ВыборкаДетальныеЗаписи.Продукция, Дата);
					Если Строка(ТипЗнч(ВозвратноеМасло)) <> "Будево" Тогда
						ВидМасла = ВозвратноеМасло;
					КонецЕсли;
