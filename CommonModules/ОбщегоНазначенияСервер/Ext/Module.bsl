...
// Проверка есть ли возврат масла в ТЦ у текущего продукта
//
// Параметры:
//  Номенклатура - СправочникСсылка.Номенклатура
//
// Возвращаемое значение:
//   Булево   - отсутствие возвратного малсла
//		или
//   Число - процент возврата масла
//
Функция ЕстьВозратМасла(Номенклатура) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Номенклатура.ПроцентВозвратаМасла КАК ПроцентВозвратаМасла
		|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.ПроцентВозвратаМасла;
	КонецЦикла;
	Возврат Ложь;
КонецФункции

// Найти полуфабрикат на который будет возвращаться масло
//
// Параметры:
//  Номенклатура - СправочникСсылка.Номенклатура
//  ДатаДокумента - Дата
//
// Возвращаемое значение:
//   Булево - отсутствие полуфабриката
//		или
//   ВыборкаДетальныеЗаписи.Материал - СправочникСсылка.Номенклатура - полуфабрикат на который возврщается масло
//
Функция УзнатьПолуфабрикат(Номенклатура, ДатаДокумента) Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	НормыРасходаСрезПоследних.Продукция = &Продукция
		|	И НормыРасходаСрезПоследних.Материал.ВидНоменклатуры = &ВидНоменклатуры";
	
	Запрос.УстановитьПараметр("ДатаДок", ДатаДокумента);
	Запрос.УстановитьПараметр("Продукция", Номенклатура);
	Запрос.УстановитьПараметр("ВидНоменклатуры", Перечисления.ВидыНоменклатуры.Полуфабрикаты);
...
// Определить процент возврата масла в ТЦ
//
// Параметры:
//  Номенклатура - СправочникСсылка.Номенклатура
//
// Возвращаемое значение:
//   Число - процент возврата
//
Функция ОпределитьПроцентВозврата(Номенклатура) Экспорт 
